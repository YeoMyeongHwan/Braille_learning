...
// 나만의 단어장을 위해 점자정보를 저장하는 데이터베이스 클래스 최대 100개의 단어 저장 가능

public class Braille_DB extends SQLiteOpenHelper {
    ...
    public Braille_DB(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
    ...
    public String insert(int count, String name, String matrix1, String matrix2, String matrix3, int reference, int reference_index){ //데이터베이스에 데이터 저장
        ...
        String result="";
        int datacount=0; //현재 입력된 데이터 갯수 카운트 변수
        ...
        if(datacount<db_manager.MAXSIZE) {
    ...
    public String getResult(){  //데이터베이스로부터 값을 읽어옴
        ...
        while(cursor.moveToNext()){
            ...
            reference = cursor.getInt(6);
            reference_index = cursor.getInt(7);
            db_manager.DB_All_insert(count, name, matrix1, matrix2, matrix3, reference, reference_index);
        }
