 ...
 */
class Quiz_Braille_long_display extends View {
    /*
 ...
 */
    ...
    public void quiz_view2_init(){//화면 초기화 함수. 화면이 이동될 때 점자를 다시 그려줌.
        ...
        dot_count = dot_quiz_word.letter_dot_count.get(page);

        if(dot_count==1) { //점자가 6개일떄
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 2; j++) {
                    text_7[i][j] = dot_quiz_word.letter_Array.get(page)[i][j];
                    textname_7 = dot_quiz_word.letter_name.get(page);
                }
            }
        }
    ...
    public Quiz_Braille_long_display(Context context) {
        super(context);
        max = dot_quiz_word.lettercount;
        random = new Random();
        quiz_view2_init();
        mMain = context;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(x!=0&&y!=0) // 손가락이 터치한 지점에 동그라미를 그림
