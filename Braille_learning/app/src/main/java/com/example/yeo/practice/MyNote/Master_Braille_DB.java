...
public class Master_Braille_DB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE BRAILLE2( _id INTEGER PRIMARY KEY AUTOINCREMENT, " + "count INTEGER," + "name STRING," + "matrix1 TEXT," + "matrix2 TEXT," + "matrix3 TEXT," + "reference INTEGER," + "reference_index INTEGER);");
    }
    ...
    public String insert(int count, String name, String matrix1, String matrix2, String matrix3, int reference, int reference_index){ //데이터베이스에 데이터 저장
        ...
        if(datacount<master_db_manager.MAXSIZE) {
    ...
    public String getResult(){  //데이터베이스로부터 값을 읽어옴
        ...
        while(cursor.moveToNext()){
            ...
            master_db_manager.DB_All_insert(count, name, matrix1, matrix2, matrix3, reference, reference_index); //데이터베이스에 저장
        }
