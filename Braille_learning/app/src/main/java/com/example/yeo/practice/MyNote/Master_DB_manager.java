...
public class Master_DB_manager {
    ...
    public int My_Note_page = 0; //현재 단어장의 페이지 수
    ...
    public void DB_All_insert(int count_temp, String name_temp, String matrix1_temp, String matrix2_temp, String matrix3_temp, int reference_temp, int reference_index_temp){
        if(size_count<MAXSIZE) {
            count[size_count] = count_temp; //점자의 칸 수를 저장하는 배열 변수
            name[size_count] = name_temp; // 점자의 이름을 저장하는 배열변수
            ...
            reference[size_count] = reference_temp; // 점자의 음성파일 주소
            reference_index[size_count] = reference_index_temp; //점자의 음성파일의 순서
            size_count++;
        }
    }
